...
const ProjectDetailKeyHours: React.FC = () => {
  ...
  const loadProject = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const data = await projectService.getProject(parseInt(id), false); // No usar simulación en vista admin
      setProject(data);
      ...
      <header className="flex w-full items-center justify-between bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 px-8 py-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="text-[14px] text-white hover:text-blue-300 transition"
          >
            Logout
          </button>
        </div>
                            ...
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center">
                                <span className="text-[14px] font-semibold text-blue-700">
                                  {app.user_name.split(' ').map((n: string) => n[0]).join('')}
                                </span>
                              </div>
                              <div>
                                <div className="text-[14px] font-medium text-black">{app.user_name}</div>
                                <div className="text-[12px] text-gray-600">Carnet: {app.user_carnet}</div>
                              </div>
                            </div>
