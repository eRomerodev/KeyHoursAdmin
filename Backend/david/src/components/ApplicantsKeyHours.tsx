...
const ApplicantsKeyHours: React.FC = () => {
  ...
  const filteredApplications = applications.filter((app) =>
    ...
    app.project_id.toString().includes(searchTerm.toLowerCase())
  );
              ...
              >
                ...
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/90 text-indigo-700 shadow-inner">
                  <span className="text-lg font-semibold">
                    {app.project_name.split(' ').map((n: string) => n[0]).join('')}
                  </span>
                </div>
                ...
                <div className="flex-1">
                  <div className="inline-flex max-w-full items-center rounded-2xl bg-white/90 px-4 py-2 text-slate-900 shadow">
                    <span className="truncate text-lg font-medium" title={app.project_name}>
                      {app.project_name}
                    </span>
                  </div>
                </div>
                ...
                {app.status === "pending" && (
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      aria-label={`Aprobar a ${app.project_name}`}
                    >
                    ...
                    <button
                      ...
                      aria-label={`Rechazar a ${app.project_name}`}
                    >
